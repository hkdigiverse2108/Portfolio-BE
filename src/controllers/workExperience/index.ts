...
import { ICommonIdValidate, IWorkExperienceValidate } from "../../type";
import { addWorkExperienceSchema, commonIdSchema, editWorkExperienceSchema } from "../../validation";
...
export const getAllWorkExperience = async (req, res) => {
  ...
  try {
    ...
    const response = await getData(workExperienceModel, criteria, {}, options);
    const totalData = await countData(workExperienceModel, criteria);
